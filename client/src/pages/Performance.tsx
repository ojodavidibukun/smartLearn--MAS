...
import { getActiveEnrollments, getStudentProgress, getStudentQuizAttempts, getQuizzes, getLessons, normalizeCourse, type Course, type CourseQuiz, type Lesson, type QuizAttempt } from '@/lib/courses';
...
export default function Performance() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setLoading(true);
      const enrollments = await getActiveEnrollments(user.uid);
      ...
      const loadedCourses = (await Promise.all(ids.map(async (id) => {
        ...
        return snapshot.exists() ? normalizeCourse(snapshot.data(), snapshot.id) : null;
      ...
      const [studentAttempts, courseData] = await Promise.all([
        ...
        Promise.all(loadedCourses.map(async (course) => ({
          ...
          quizzes: await getQuizzes(course.id!, true),
  ...
  const completedAttempts = useMemo(() => attempts.filter((attempt) => attempt.status !== 'in_progress'), [attempts]);
  const topicScores = useMemo<TopicScore[]>(() => {
    const totals = new Map<string, { correct: number; total: number }>();
    completedAttempts.forEach((attempt) => Object.entries(attempt.topicScores || {}).forEach(([topic, score]) => {
  ...
  const weakTopics = topicScores.filter((item) => item.total >= 2 && item.percentage < 60);
  const quizAverage = completedAttempts.length ? Math.round(completedAttempts.reduce((sum, attempt) => sum + Number(attempt.score || 0), 0) / completedAttempts.length) : null;
  ...
  const recentAttempts = [...completedAttempts].sort((a, b) => completedAtValue(b) - completedAtValue(a)).slice(0, 5);

  if (profileLoading) return <MainLayout><div className="container py-8">Loading...</div></MainLayout>;
  if (profile?.role === 'lecturer') return <MainLayout><div className="container py-8"><h1 className="text-3xl font-bold">Facilitator content</h1><p className="mt-2 text-muted-foreground">Personal performance insights belong to learners. Manage your courses and learning content from the facilitator dashboard.</p></div></MainLayout>;
  ...
  return <MainLayout><div className="container py-8 space-y-8">
    ...
    <div className="grid gap-4 md:grid-cols-3"><Card className="p-5"><p className="text-sm text-muted-foreground">Quiz average</p><p className="mt-2 text-3xl font-bold">{quizAverage === null ? 'No data yet' : `${quizAverage}%`}</p><p className="mt-1 text-xs text-muted-foreground">Based on submitted quizzes</p></Card><Card className="p-5"><p className="text-sm text-muted-foreground">Course progress</p><p className="mt-2 text-3xl font-bold">{totalLessons ? `${Math.round((completedLessons / totalLessons) * 100)}%` : 'No data yet'}</p><p className="mt-1 text-xs text-muted-foreground">{totalLessons ? `${completedLessons} of ${totalLessons} published lessons completed` : 'No published lessons available'}</p></Card><Card className="p-5"><p className="text-sm text-muted-foreground">Quizzes completed</p><p className="mt-2 text-3xl font-bold">{completedAttempts.length || 'No data yet'}</p></Card></div>
    ...
    <Card className="p-5"><div className="mb-4"><h2 className="font-semibold">Topics to Review</h2><p className="text-sm text-muted-foreground">Topics below 60% based on your own submitted quiz answers.</p></div>{weakTopics.length === 0 ? <p className="text-sm text-muted-foreground">No weak topics have been identified.</p> : <div className="flex flex-wrap gap-2">{weakTopics.map((topic) => <Badge key={topic.topic} variant="destructive">{topic.topic} · {topic.percentage}%</Badge>)}</div>}</Card>
